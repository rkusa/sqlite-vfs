...
pub mod asynchronous {
    ...
    impl Connection {
        ...
        pub async fn send(&mut self, encode: impl FnOnce(&mut Vec<u8>)) -> io::Result<()> {
            self.send_buffer.shrink_to(8192);
            self.send_buffer.clear();
        ...
        pub async fn receive(&mut self) -> io::Result<Option<&[u8]>> {
            self.recv_buffer.resize(8192, 0);

            let mut chunk_len = 0;
            loop {
                let n = self.stream.read(&mut self.recv_buffer[chunk_len..]).await?;
                if n == 0 {
                    // connection got closed
                    if chunk_len > 0 {
                        return Err(io::Error::new(
